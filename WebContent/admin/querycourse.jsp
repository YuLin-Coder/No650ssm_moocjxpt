...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>课程管理 <span class="c-gray en">&gt;</span> 课程查询 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">课程查询</th>
				</tr>
				<tr class="text-c">
			...
			<c:forEach items="${courseList}" var="course">
				<tr class="text-c">
					...
					<td class="center">${course.clazzname}</td>
					<td class="center">${course.credits}</td>
					<td class="center">${course.timelong}</td>
					<td class="center">${course.addtime}</td>
					<td class="center">${course.memo}</td>
				</tr>
			</c:forEach>
		</table>
		...
		<div class="text-c">
			<form action="course/queryCourseByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						...
						<option value="credits">按学分查询</option>
						<option value="timelong">按学时查询</option>
