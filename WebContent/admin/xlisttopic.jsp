...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>学生留言管理 <span class="c-gray en">&gt;</span> 学生留言列表 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">学生留言列表</th>
				</tr>
				<tr class="text-c">
					<th>学生</th>
					<th>视频</th>
			...
			<c:forEach items="${topicList}" var="topic">
				<tr class="text-c">
					...
					<td>${topic.vedioname}</td>
					<td>${topic.contents}</td>
					<td>${topic.addtime}</td>
					<td>${topic.status}</td>
					<td>${topic.reps}</td>
					<td><c:if test="${topic.status eq '未回复' }">
							<a href="topic/getTopicById.action?id=${topic.topicid}" title="回复">回复</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</table>
